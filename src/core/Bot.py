...
__all__ = ("Shinchan", "bot")


on_startup: List[Callable[["Shinchan"], Coroutine]] = []


class Shinchan(commands.AutoShardedBot):
    ...
    def reboot(self):
        return os.system("pm2 reload Shinchan")
    ...
    async def on_ready(self):
        print(f"[Shinchan] Logged in as {self.user.name}({self.user.id})")
