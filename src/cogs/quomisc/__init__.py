...
class Quomisc(Cog, name="quomisc"):
        ...
        if search is None:
            e = Embed(
                ...
                description=(
                    ...
                    "_The Shinchan Legacy repo is now open-source with video tutorial._ https://www.youtube.com/watch?v=7E2hB0sX0hg"
                ),
                color=config.COLOR
            )
    ...
    async def invite(self, ctx: Context):
        ...
        v.add_item(
            discord.ui.Button(
                style=discord.ButtonStyle.link, label="Invite Shinchan (Me)", url=self.bot.config.BOT_INVITE, row=1
            )
        )
        v.add_item(
            discord.ui.Button(
                style=discord.ButtonStyle.link, label="Invite Shinchan Legacy", url=self.bot.config.PRO_LINK, row=2
            )
        )
        v.add_item(
            discord.ui.Button(
    ...
    async def make_private_channel(self, ctx: Context) -> discord.TextChannel:
        ...
        channel = await guild.create_text_channel(
            "Shinchan-private", overwrites=overwrites, reason=f"Made by {str(ctx.author)}"
        )
        ...
        e.add_field(
            ...
            inline=False,
            value="This channel is made for Shinchan to send important announcements and activities that need your attention. If anything goes wrong with any of my functionality I will notify you here. Important announcements from the developer will be sent directly here too.\n\nYou can test my commands in this channel if you like. Kindly don't delete it , some of my commands won't work without this channel.",
        )
        e.add_field(
    ...
    async def setup_cmd(self, ctx: Context):
        ...
        Shinchan requires manage channels and manage wehooks permissions for this to work.
    ...
    def format_commit(commit):  # source: R danny
        ...
        return f"[`{short_sha2}`](https://github.com/CycloneAddons/Shinchan-Legacy/commit/{commit.hex}) {truncate_string(short,40)} ({offset})"
    ...
    async def about(self, ctx: Context):
        ...
        embed.title = f"{emote.bot} Shinchan - The Legacy Continues"
        ...
        embed.add_field(
             ...
             value=f"Originally created by [**{str(legacy_owner)}**](https://github.com/deadaf), whose vision built the foundation of Shinchan.",
             inline=False,
          )
    ...
    async def vote(self, ctx: Context):
        e = self.bot.embed(ctx, title="Vote for Shinchan")
        e.description = (
            "**Rewards**\n"
    ...
    async def contributors(self, ctx):
        ...
        url = f"https://api.github.com/repos/quotientbot/Shinchan-Bot/contributors"
