...
class PremiumCog(Cog, name="Premium"):
    ...
    async def pstatus(self, ctx: Context):
        """Get your Shinchan Premium status and the current server's."""
        ...
        embed = self.bot.embed(ctx, title="Shinchan Premium", url=f"{self.bot.config.WEBSITE}")
    ...
    async def prefresh(self, ctx: Context):
        ...
        embed = self.bot.embed(ctx, title="Shinchan Legacy Premium Refreshed", url=f"{self.bot.config.WEBSITE}")
        embed.add_field(name="Server", value=btext, inline=False)
        embed.set_thumbnail(url=ctx.guild.me.display_avatar.url)
    ...
    async def premium(self, ctx: Context):
        """Checkout Shinchan Premium Plans."""
        _e = discord.Embed(color=0x00FFB3)
        _e.title = "💎 Welcome to Shinchan Legacy!"
        _e.description = (
            "__**Perks you get with Shinchan Legacy (completely FREE!)**__\n\n"
            f"{emote.white_arrow} Access to `Shinchan Legacy` bot.\n"
            ...
            "_We provide this for free to honor the memory of Rohit, the original creator of Shinchan. "
            "If you love what we do and want to support us to keep this alive, you can donate using the button below._"
        )
        _e.set_footer(text="Thank you for being part of the Shinchan Legacy ❤️")
    ...
    async def on_guild_premium_timer_complete(self, timer: Timer):
        ...
        if (_ch := _g.private_ch) and _ch.permissions_for(_ch.guild.me).embed_links:
            _e = discord.Embed(
                color=discord.Color.red(), title="⚠️__**Shinchan Legacy Subscription Ended**__⚠️", url=config.SERVER_LINK
            )
            _e.description = (
                "This is to inform you that your subscription of Shinchan Legacy has been ended.\n\n"
    ...
    async def on_premium_purchase(self, txnId: str):
        ...
        with suppress(discord.HTTPException, AttributeError):
            _e = discord.Embed(
                color=discord.Color.gold(), description=f"Thanks **{member}** for purchasing Shinchan Premium."
            )
        ...
        _e = discord.Embed(
            ...
            title="Shinchan Legacy Purchase Successful!",
            url=self.bot.config.SERVER_LINK,
            description=(
                f"{random_greeting()} {member.mention},\n"
                f"Thanks for purchasing Shinchan Premium. Your server **{upgraded_guild}** has access to Shinchan Legacy features until `{_guild.premium_end_time.strftime('%d-%b-%Y %I:%M %p')}`.\n\n"
                "[Click me to Invite Shinchan Legacy Bot to your server](https://discord.com/oauth2/authorize?client_id=902856923311919104&scope=applications.commands%20bot&permissions=21175985838)\n"
            ),
        )

        if member not in self.bot.server.members:
            _e.description += f"\n\n[To claim your Premium Role, Join Shinchan HQ]({self.bot.config.SERVER_LINK})."
