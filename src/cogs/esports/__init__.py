...
class ScrimManager(Cog, name="Esports"):
    ...
    async def idp(self, ctx: Context, *, data: str):
        """
        Send Scrims ID/Password format.
        ...
        if len(parts) != 7:
            return await ctx.error(
                "❌ **Invalid IDP format!**\n\n"
                f"Use:\n`{ctx.prefix}idp "
                "<scrims name> | <match no> | <room id> | "
                "<password> | <map> | <start time> | <role>`\n\n"
                "**Example:**\n"
                f"`{ctx.prefix}idp TORMENTA T3 SCRIMS | 3 | "
                "2282720 | T3M3 | MIRAMAR | 4:40 | @Admin`"
            )

        scrims_name, match_no, room_id, password, map_name, start_time, role_text = parts
        room_id = truncate_string(room_id, 100)
        password = truncate_string(password, 100)
        map_name = truncate_string(map_name, 100)
        scrims_name = truncate_string(scrims_name, 256)
        match_no = truncate_string(match_no, 100)
        start_time = truncate_string(start_time, 100)

        role_to_ping = None
        if ctx.guild:
            if role_text.startswith("<@&") and role_text.endswith(">"):
                try:
                    role_to_ping = ctx.guild.get_role(int(role_text[3:-1]))
                except ValueError:
                    role_to_ping = None
        ...
        embed.description = (
            ...
            "• Wasting a slot will result in a **Scrims ban**.\n\n"
            "• Screenshots submission is compulsory within 30 minutes "
            "after completion of both matches or you will get **NO points**.\n\n"
            "> **--- NO SS = NO POINTS ---**"
        )
        embed.set_thumbnail(url=server_logo)

        role_mention = role_to_ping.mention if role_to_ping else ""
        embed.add_field(
            name="\u200b",
            value=f"Thanks & Regards,\n**{server_name}**\n{role_mention}",
            inline=False,
        )
        embed.set_footer(text=server_name, icon_url=server_logo)
        embed.timestamp = self.bot.current_time + timedelta(minutes=30)

        view = IdpView(room_id, password, map_name)
        msg = await ctx.send(
    ...
    async def set_eztag(self, ctx: Context, *, channel: QuoTextChannel):
        ...
        if count == 1 and not guild.is_premium:
            return await ctx.error(
                f"Upgrade your server to Shinchan Premium to setup more than 1 EasyTag channel.\n[Click Me to Purchase]({self.bot.prime_link})"
            )
        ...
        role = discord.utils.get(ctx.guild.roles, name="Shinchan-tag-ignore")
        if not role:
            role = await ctx.guild.create_role(
                name="Shinchan-tag-ignore", color=self.bot.color, reason=f"Created by {ctx.author}"
            )
    ...
    async def tagcheck_set(self, ctx: Context, channel: discord.TextChannel, mentions: int = 4):
        ...
        if count == 1 and not guild.is_premium:
            return await ctx.error(
                f"Upgrade your server to Shinchan Premium to setup more than 1 Tagcheck channel.\n[Click Me to Purchase]({self.bot.prime_link})"
            )
