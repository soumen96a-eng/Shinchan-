        ...
        if _v.custom_id:
            ...
            if not self.view.record.ss_type == SSType.yt:
                if not await self.ctx.is_premium_guild():
                    return await self.ctx.error("You need Shinchan Legacy to set this filter. (Use `qperks`)", 4)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not await self.ctx.is_premium_guild():
            if await SSVerify.filter(guild_id=self.ctx.guild.id).exists():
                return await self.ctx.premium_mango("You need Shinchan Premium to setup more than 1 ssverify.")
