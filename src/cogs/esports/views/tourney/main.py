...
class TourneyManager(EsportsBaseView):
    ...
    async def initial_embed(self) -> discord.Embed:
        ...
        _e = discord.Embed(
            color=self.bot.color, title="Shinchan Smart Tournament Manager", url=self.bot.config.SERVER_LINK
        )
        ...
        _e.set_footer(
            text="Shinchan Legacy allows unlimited tournaments.",
        ...
        if not await self.ctx.is_premium_guild():
            if await Tourney.filter(guild_id=self.ctx.guild.id).count() >= 1:
                return await self.ctx.error(
                    f"You need [Shinchan Premium](https://quotientbot.xyz/premium) to create more than one tournament.\n"
    ...
    async def download_excel_data(self, interaction: discord.Interaction, button: discord.Button):
        ...
        # if not await self.ctx.is_premium_guild():
        #     return await self.ctx.error(
        #         "You need Shinchan Premium to download Ms Excel file containing all the "
