...
class BaseScrim(BaseModel):
    ...
    async def validate_perms(self, bot: Shinchan) -> Tuple[bool, Union[bool, str]]:
        ...
        if not reg_channel:
            return False, "Shinchan can't see your registration channel. Give Perms."

        _p = reg_channel.permissions_for(guild.me)
        if not all((_p.manage_channels, _p.manage_permissions, _p.manage_messages)):
            return False, "Shinchan can't manage this registration channel. Give Perms."

        role = guild.get_role(self.role_id)
        if not role:
            return False, "Shinchan couldn't find your sucess role."
    ...
    async def create_scrim(self, bot: Shinchan):
    ...
    async def update_scrim(self, bot: Shinchan):
    ...
    async def __check_bot_perms(self, bot: Shinchan):
        ...
        if not all((g_perms.manage_channels, g_perms.manage_roles, g_perms.manage_messages)):
            return False, "Shinchan needs manage channels, manage roles permission."

        if not all((g_perms.add_reactions, g_perms.embed_links)):
            return False, "Shinchan needs add reacions & embed links permission."
